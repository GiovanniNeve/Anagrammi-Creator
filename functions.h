...
int findDuplicates(std::string word) {
    ...
    int strLen = word.length();     /* Get the word length */
    std::map<char, int> map;    /* Store word's letter in a map */

    for(int i=0; i<strLen; i++) {
        map[word[i]] += 1;  /* If there are duplicates some letters will result higher than 1 */
    }
...
int findFactorial(int number, int result) {     /* Calculate factorial number with recursion*/
...
std::vector<std::string> makeAnagrams(std::string word) {
    ...
    int nDuplicates = findDuplicates(word);     /* Store the number of duplicates */
